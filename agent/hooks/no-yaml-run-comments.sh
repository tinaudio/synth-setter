...
in_scope() {
  ...
  case "$1" in
    ...
    */.github/workflows/*.yaml|*/.github/workflows/*.yml) return 0 ;;
    src/synth_setter/configs/skypilot_launch/compute/*.yaml|src/synth_setter/configs/skypilot_launch/compute/*.yml) return 0 ;;
    */src/synth_setter/configs/skypilot_launch/compute/*.yaml|*/src/synth_setter/configs/skypilot_launch/compute/*.yml) return 0 ;;
  esac
  return 1
}
...
emit_block_message() {
  ...
  local file_path="$1" violations="$2"
  {
    ...
    echo "    run: |"
    echo "      sed -i \"s|...|...|\" src/synth_setter/configs/skypilot_launch/compute/runpod/smoke.yaml"
    echo

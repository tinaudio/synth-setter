...
def _build_surge_xt_smoke_cfg(accelerator: str, param_spec_name: str) -> DictConfig:
    """Construct the Surge XT smoke-test config without the accelerator availability gate.

    Composes ``train.yaml`` with ``experiment=surge/fake_oracle`` and bakes in the minimal
    overrides needed to train-smoke-test on the dataset generated by
    :func:`surge_xt_smoke_datasets`. Used both by the :func:`cfg_surge_xt_global` fixture
    (where the parametrized ``accelerator`` is host-checked upstream) and by the
    ``configs/experiment/surge/test-mps.yaml`` equality test (where the cfg must be built
...
def cfg_surge_xt_global(accelerator: str, param_spec_name: str) -> DictConfig:
    ...
    """
    return _build_surge_xt_smoke_cfg(accelerator, param_spec_name)

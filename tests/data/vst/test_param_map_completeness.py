...
def test_committed_map_vst_names_match_after_preset_settlement(
    committed_map: SynthParamMap,
) -> None:
    """DawDreamer and Pedalboard record the same active preset-specific labels.

    :param committed_map: Packaged map.
    """
    for semantic_key, identity in committed_map.params.items():
        assert identity.dawdreamer.name == identity.pedalboard.name, (

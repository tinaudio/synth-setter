...
def _assert_all_pairs_audio_metrics_within_thresholds(
    ...
    # Backfill mode: thresholds downgraded to warnings so emission always
    # happens — we want to capture pre-fix bug-fire runs in the bench JSON,
    # not just runs that pass thresholds.
    if worst_mss[0] >= _MSS_MAX:
        log.warning(
            "%s: worst mss=%.4f at pair %s exceeds %s",
            label_prefix, worst_mss[0], worst_mss[1:], _MSS_MAX,
        )
    if worst_wmfcc[0] >= _WMFCC_MAX:
        log.warning(
            "%s: worst wmfcc=%.4f at pair %s exceeds %s",
            label_prefix, worst_wmfcc[0], worst_wmfcc[1:], _WMFCC_MAX,
        )
    if worst_sot[0] >= _SOT_MAX:
        log.warning(
            "%s: worst sot=%.4f at pair %s exceeds %s",
            label_prefix, worst_sot[0], worst_sot[1:], _SOT_MAX,
        )
    if worst_rms_cos[0] <= _RMS_MIN_COSINE:
        log.warning(
            "%s: worst rms cosine=%.4f at pair %s below %s",
            label_prefix, worst_rms_cos[0], worst_rms_cos[1:], _RMS_MIN_COSINE,
        )
...
def test_datasets_from_hardcoded_params_are_identical(
    ...
    # Backfill mode (#489): swallow round-trip assertion failures so emission
    # downstream still runs. Compute round-trip metrics inline as a fallback
    # if the helper raises before returning.
    round_trip_metrics: RoundTripMetrics | None = None
    try:
        ...
        log.warning("backfill: round-trip assertion failed, computing metrics inline: %s", e)
        mss_values, wmfcc_values, sot_values, rms_cos_values = [], [], [], []
        for i in range(num_samples):
            mss, wmfcc, sot, rms_cos = _audio_metrics(expected_audio[i], actual_audio[i])
            mss_values.append(float(mss))
            wmfcc_values.append(float(wmfcc))
            sot_values.append(float(sot))
            rms_cos_values.append(float(rms_cos))
        round_trip_metrics = RoundTripMetrics(
            ...
            sot_max=max(sot_values),
            rms_cos_min=min(rms_cos_values),
            mel_mean_abs=float(np.mean(np.abs(actual_mel - expected_mel))),
            num_samples=num_samples,
        )

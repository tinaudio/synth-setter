...
def test_pyfdn_acceptance_lance_reader_rerender_round_trip(
    ...
    :param synth_name: Registered plain-topology pyFDN identity under test.
    :param param_spec: The spec that identity resolves to.
    """
    render = RenderConfig(
        synth=SynthSpec(
            ...
            plugin_state_path="",
            synth_version="0.4.2",
            param_spec_sha256=pyfdn_param_spec_sha256(param_spec),
        ),
    ...
    renderer = PyFDNRenderer(param_spec_name=ParamSpecName(synth_name))
    native_rerender = renderer.render(native_params)
    ...
    decoded_rerender = renderer.render(decoded)

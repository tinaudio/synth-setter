...
def _guarded_cfg(encoder_target: str, force: bool = False) -> DictConfig:
    ...
    """
    cfg = _cfg(task_name="flow-simple", upload_checkpoints_uri=_CKPT_SLOT_URI)
    with open_dict(cfg):
...
def _upload_guarded_checkpoint(cfg: DictConfig, checkpoint: Path) -> str | None:
    """Upload a guarded checkpoint through a stable shared override.

    :param cfg: Fingerprinted train configuration.
    :param checkpoint: Local checkpoint to upload.
    :returns: The shared checkpoint URI, or ``None`` when the guard refuses it.
    """
    return _upload_best_checkpoint(cfg, str(checkpoint), _TRAINING_RUN_ID, _LAUNCH_UUID)
...
def test_upload_best_checkpoint_writes_fingerprint_sidecar(
    ...
    uri = _upload_guarded_checkpoint(_guarded_cfg(_ENCODER_A), _new_ckpt(tmp_path))
...
def test_upload_best_checkpoint_mismatched_fingerprint_refuses_overwrite(
    ...
    uri = _upload_guarded_checkpoint(_guarded_cfg(_ENCODER_B), _new_ckpt(tmp_path))
...
def test_upload_best_checkpoint_force_flag_overwrites_despite_mismatch(
    ...
    uri = _upload_guarded_checkpoint(_guarded_cfg(_ENCODER_B, force=True), _new_ckpt(tmp_path))

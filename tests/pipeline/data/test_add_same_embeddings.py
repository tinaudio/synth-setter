...
def test_main_same_mode_derives_resume_cache_keyed_by_same_checkpoint(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A SAME-mode run keys its default resume cache on the SAME weights, not CLAP.

    :param tmp_path: Per-test dataset and cache root.
    :param monkeypatch: Fixture pinning the cache root and injecting a fake loader.
    """
    from structlog.testing import capture_logs

    spec = build_lance_smoke_spec()
    uri = tmp_path / "same-cache.lance"
    write_minimal_lance_shard(uri, spec)
    ...
    version = lance.dataset(str(uri)).version
    ...
    same_key = _default_resume_cache(str(uri), version, DEFAULT_LANCE_BATCH_SIZE, "/models/same-s")
    clap_key = _default_resume_cache(
        str(uri), version, DEFAULT_LANCE_BATCH_SIZE, "laion/clap-htsat-unfused"
    )
    assert configured["path"] == str(same_key)
    assert configured["path"] != str(clap_key)

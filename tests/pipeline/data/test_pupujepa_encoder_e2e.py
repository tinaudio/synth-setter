...
def test_real_pupujepa_weights_add_embeddings_and_online_consumers_match(
    ...
    try:
        with initialize_config_module(version_base="1.3", config_module="synth_setter.configs"):
            ...
            cached_config = compose(
                ...
                overrides=[
                    "experiment=torchsynth/flow",
                    "conditioning=pupujepa_tiny",
                    "trainer=cpu",
                ],
            )
        online = hydra.utils.instantiate(online_config.model.encoder).eval()
        cached = hydra.utils.instantiate(cached_config.model.encoder).eval()
    finally:
        GlobalHydra.instance().clear()
    ...
    dataset_path = tmp_path / "pupujepa-real.lance"
    ...
    table = lance.dataset(dataset_path).to_table(
        columns=[PUPUJEPA_TINY_FIELD, f"{PUPUJEPA_TINY_FIELD}_vec"]
    )
    offline_sequence = table.column(PUPUJEPA_TINY_FIELD).combine_chunks().to_numpy_ndarray()
    offline_vector = np.stack(
        table.column(f"{PUPUJEPA_TINY_FIELD}_vec").to_numpy(zero_copy_only=False)
    )
    with torch.inference_mode():
        online_sequence = online.embed(mono)
        online_conditioning = online(mono)
        cached_conditioning = cached(torch.from_numpy(offline_sequence))

    assert offline_sequence.shape == (1, PUPUJEPA_EMBEDDING_DIM, 100)

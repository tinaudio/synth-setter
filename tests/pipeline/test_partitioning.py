...
is the imperative shell that reads ``OVERRIDE_SKYPILOT_NODE_RANK`` /
``OVERRIDE_SKYPILOT_NUM_NODES`` from the environment; tests inject env
...
class TestReadRankWorldFromEnv:
    ...
    def test_returns_rank_world_tuple_when_env_valid(
        ...
        monkeypatch.setenv("OVERRIDE_SKYPILOT_NODE_RANK", "2")
    ...
    def test_only_rank_missing_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(ValueError, match="OVERRIDE_SKYPILOT_NODE_RANK"):
            read_rank_world_from_env()
    ...
    def test_only_world_missing_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(ValueError, match="OVERRIDE_SKYPILOT_NUM_NODES"):
            read_rank_world_from_env()
    ...
    def test_non_integer_rank_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setenv("OVERRIDE_SKYPILOT_NODE_RANK", "abc")
        monkeypatch.setenv("OVERRIDE_SKYPILOT_NUM_NODES", "1")
        with pytest.raises(ValueError, match="OVERRIDE_SKYPILOT_NODE_RANK"):
            read_rank_world_from_env()
    ...
    def test_world_zero_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setenv("OVERRIDE_SKYPILOT_NODE_RANK", "0")
        monkeypatch.setenv("OVERRIDE_SKYPILOT_NUM_NODES", "0")
        with pytest.raises(ValueError, match="world=0"):
            read_rank_world_from_env()

...
def test_torchsynth_ffn_experiment_monitors_lsd_with_runnable_input_length() -> None:
    ...
    The encoder's single global magnitude-FFT head scales O(in_dim**2)
    (``LazyLinear(in_dim // 2)`` on flattened conv features), so the experiment must
    not inherit the 4 s datamodule default until the time-frequency front-end (#1848)
    lands.
    """
    with initialize_config_module(version_base="1.3", config_module="synth_setter.configs"):

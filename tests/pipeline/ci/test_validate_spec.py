...
class TestValidateStructure:
    ...
    def test_missing_r2_returns_error(self) -> None:
        """Spec missing the nested ``r2`` block is rejected.

        Locks the model-derived required set: ``r2`` is the single carrier for the
        bucket / prefix_root / prefix triple post-nesting, so the hand-rolled
        required list used to miss the legacy flat keys but now catches the
        nested model field by deriving from ``DatasetSpec.model_fields``.
        """
        spec = _make_valid_spec()
        del spec["r2"]
        errors = validate_structure(spec)

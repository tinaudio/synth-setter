...
from tests.pipeline.data.test_lance_finalize import (
    stage_all_shards,
    stage_duplicate_attempt,
    staging_file,
)
...
def test_validate_all_lance_shards_falls_back_to_next_healthy_attempt(
    fake_r2_remote: Path, tmp_path: Path
) -> None:
    """Validation accepts a healthy candidate after invalidating an earlier corrupt one.
    ...
    spec = tiny_lance_spec()
    stage_all_shards(spec, tmp_path)
    stage_duplicate_attempt(spec, tmp_path, 0, attempt_uuid="healthy", value_offset=7000)
...
def test_validate_all_lance_shards_aggregates_malformed_stats_sidecar(
    ...
    assert errors[0].startswith(
        "shard-000002.lance: shard 2 has no healthy staged-valid attempt: "
        "shard 2 attempt pod-a-u0002: "
    )

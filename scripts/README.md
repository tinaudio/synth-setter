...
| `compute_audio_metrics.py`     | Compute multi-scale spectrogram and MFCD audio metrics on predicted vs target audio  | `python scripts/compute_audio_metrics.py --help`                            |
| `docker_entrypoint.sh`         | Docker container entrypoint dispatching on the `MODE` env var (idle, generate, etc.) | `MODE=idle bash scripts/docker_entrypoint.sh`                               |

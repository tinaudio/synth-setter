...
evaluates held-out param MSE and audio LSD via Euler sampling. Overtraining a
...
def evaluate(
    ...
    """Report held-out param MSE (model space) and audio LSD via Euler sampling.
    ...
    :returns: Mean ``param_mse`` and ``lsd`` over the held-out batches.
    """
    ...
    _LOG.info("[%s] param_mse=%.4f lsd=%.3f", label, result["param_mse"], result["lsd"])
    return result

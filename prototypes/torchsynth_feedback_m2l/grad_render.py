"""Gradient-enabled TorchSynth rendering for the m2l simulator-feedback spike (#2557).

Verbatim reuse of the sibling mel-cell spike's renderer (#2553,
``prototypes/torchsynth_feedback/grad_render.py``) so both torchsynth cells
share one simulator grad path. Sibling of
:func:`synth_setter.data.torchsynth_datamodule.render_torchsynth` that keeps
autograd connected from the normalized parameter batch to the rendered audio.
The production renderer copies values into ``ModuleParameter.data`` and renders
under ``torch.no_grad()``; here the parameter tensors are substituted via
``torch.func.functional_call`` so the graph flows through ``Voice.output()``.
"""

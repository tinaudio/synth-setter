...
| Launcher's client settings             | `SKYPILOT_API_SERVER_ENDPOINT`, optional `SKYPILOT_SERVICE_ACCOUNT_TOKEN`                         | Resolves from the same `.env` before process env, projects into the launcher process, clears SkyPilot's import-time endpoint cache, and verifies server auth before provisioning.                                                                                                                                                                                                                    |
...
- **No `.env` and no process env keys:** launcher fails fast with `No object storage settings resolved. Set SYNTH_SETTER_STORAGE_* in process env (e.g. via 'docker run -e SYNTH_SETTER_STORAGE_*=...') or populate <path>.`
- **`.env` exists but is empty or has only comments:** treated as no-keys-resolved (same path as above).
- **Some keys resolved, some missing:** the launcher fails before submission because `StorageSettings` requires access key id, secret access key, and endpoint URL before projecting the current rclone env block.
- **SkyPilot client auth is malformed or rejected:** typed client settings reject invalid endpoints and orphaned/malformed service tokens; a remote health check then stops invalid credentials before provider bootstrap or job submission.

...
| ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Runner (`src/synth_setter/cli/generate_dataset.py`'s `main()`, via `spec_io.write_spec_locally`) | local: `<workspace>/data/<task_name>/<run_id>/metadata/input_spec.json` (`main()` passes `_OPERATOR_WORKSPACE` via `synth_setter.workspace.operator_workspace()` — env override → checkout marker → CWD; `cfg.paths.output_dir` is pinned to the same value as a shim for `${hydra:runtime.output_dir}` resolution and is not read back as the artifact anchor) | **Operator-side artifact**: an on-disk copy of the frozen spec available before any dispatch happens. Used for offline inspection, reproducing a run from a saved spec, and as a stable file path the workflow YAML can grep when extracting the canonical R2 URI. Anticipates §3a's target `metadata/` layout while the R2 destination remains flat per [#385](https://github.com/tinaudio/synth-setter/issues/385). | The operator / CI workflow (e.g. `generate-dataset-shards.yaml`'s spec_uri step).                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

...
| Launcher (`src/synth_setter/pipeline/skypilot_launch.py`'s `upload_spec_to_r2()`) | `skypilot-launcher-specs/{job_name}.json` | **Transport / validation handoff**: surfaces the materialized spec to downstream CI from the launcher host without going through SkyPilot's file_mounts (workaround for [#749](https://github.com/tinaudio/synth-setter/issues/749) — RunPod backend rejects `task.update_file_mounts(...)` with a pubkey-overflow at pod create). The worker pod also receives the URI as `WORKER_SPEC_URI` env. | The `validate-spec` / `validate-shard` CI workflows read this copy via the `spec_uri` output of `generate-dataset-shards.yaml`. The worker process itself does **not** consume `WORKER_SPEC_URI`: `synth-setter-generate-dataset-from-hydra` (`src/synth_setter/cli/generate_dataset.py:from_hydra`) recomposes the spec from the Hydra overrides the launcher pins into its `run:` command, so the env var is informational on the pod and load-bearing only for validate-time consumers. |

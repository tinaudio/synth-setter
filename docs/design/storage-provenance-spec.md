...
└── skypilot-launcher-specs/{job_name}.json   # Transport-only; not part of the dataset footprint. See §3a "Materialized spec: three destinations".
```
...
#### Materialized spec: three destinations, three purposes

The `DatasetSpec` JSON is written from three distinct call sites, with three distinct purposes. This split is intentional, not a duplication bug:

| Writer                                                                                           | Location                                                                 | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Consumers                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runner (`src/synth_setter/cli/generate_dataset.py`'s `main()`, via `spec_io.write_spec_locally`) | local: `<output_dir>/data/<task_name>/<run_id>/metadata/input_spec.json` | **Operator-side artifact**: an on-disk copy of the frozen spec available before any dispatch happens. Used for offline inspection, reproducing a run from a saved spec, and as a stable file path the workflow YAML can grep when extracting the canonical R2 URI. Anticipates §3a's target `metadata/` layout while the R2 destination remains flat per [#385](https://github.com/tinaudio/synth-setter/issues/385).                                                                                                   | The operator / CI workflow (e.g. `generate-dataset-shards.yaml`'s spec_uri step).                                                                                                                                                                                                                                                                                                                                                                                            |
| Worker (`spec_io.upload_spec`, called from `generate_dataset.run()`)                             | `{spec.r2.prefix}input_spec.json` (via `spec.r2.input_spec_uri()`)       | **Provenance**: the canonical, authoritative frozen spec, archived next to the shards it parameterized. The same object that §3a's table lists as "Input spec" (target path `metadata/input_spec.json` — flat under `r2.prefix` today per [#385](https://github.com/tinaudio/synth-setter/issues/385)). A later phase of the workflow-spec-upload-delegation series will also upload from `main()` (after threading rclone env loading out of `dispatch_via_skypilot`); both writes will be idempotent at the same key. | None at runtime yet. Long-lived archive for post-hoc inspection / re-validation. The future `finalize` stage and `status` command ([#406](https://github.com/tinaudio/synth-setter/issues/406), [#72](https://github.com/tinaudio/synth-setter/issues/72)) will read this copy.                                                                                                                                                                                              |
| Launcher (`src/synth_setter/pipeline/skypilot_launch.py`'s `upload_spec_to_r2()`)                | `skypilot-launcher-specs/{job_name}.json`                                | **Transport**: ships the spec from the launcher host into the SkyPilot/RunPod worker pod. Workaround for [#749](https://github.com/tinaudio/synth-setter/issues/749) (SkyPilot's RunPod backend rejects `task.update_file_mounts(...)` with a pubkey-overflow at pod create).                                                                                                                                                                                                                                           | The `validate-spec` / `validate-shard` CI workflows read this copy via the `spec_uri` output of `generate-dataset-shards.yaml`. The worker process itself does **not** consume `WORKER_SPEC_URI`: `synth-setter-generate-dataset-from-hydra` (`from_hydra` in `cli/generate_dataset.py`) rebuilds the spec from the Hydra overrides the launcher pins into its `run:` command, so the env var is informational on the pod and load-bearing only for validate-time consumers. |

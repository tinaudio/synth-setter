...
- Workers may write only per-attempt metadata under `metadata/workers/`, plus conditional claim/complete updates to the one shared Lance claims table `metadata/shard-claims.lance` (`pipeline/shard_claims.py`), except Lance workers may also write uncommitted fragment data under `train.lance/data/`, `val.lance/data/`, or `test.lance/data/`. Dataset reopen may copy a finalized card's selected uncommitted train fragments into a fresh destination's `train.lance/data/`; it writes no Lance manifests or transactions. Workers never write final Lance manifests, transactions, `metadata/dataset.complete`, or dataset-level `stats.npz`. *(future state — current workers write directly to `data/{config_id}/{run_id}/`; see [#406](https://github.com/tinaudio/synth-setter/issues/406))*

...
The launcher's `skypilot-launcher-specs/` prefix is **outside** the canonical `data/{dataset_config_id}/{dataset_wandb_run_id}/` tree on purpose — it's a per-job transport queue, not part of the dataset's R2 footprint. Keep the launcher's transport upload distinct from the runner's canonical upload; do not consolidate them.

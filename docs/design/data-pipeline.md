...
**Stats-only finalize:** `finalize_lance` streams only the train shards from R2 through Welford to compute `stats.npz` and leaves every `shard-*.lance` in place under the run prefix — it writes no merged `train/val/test.lance` (that merge re-shipped the whole dataset through the finalize host and hit bandwidth limits). Training consumes the per-shard datasets directly via `ShardedLanceVSTDataModule`, which resolves each split's shard subset from `input_spec.json`'s `split_shard_ranges` and reads them as one logical dataset (typically over an `rclone mount` of the run prefix). A future fragment-metadata-commit finalize could instead join the shards into one manifest with no data copy ([§12](#12-open-questions-risks--limitations)).

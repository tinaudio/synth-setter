...
and `meanaudio_16k`) stored as fixed-shape tensors. All are derived from the audio column and
...
`matpac_plus_vec`, or `meanaudio_16k_vec`); when `build_index=true`, IVF_PQ indexes `clap`, `ssondo`, and the

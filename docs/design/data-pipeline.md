...
- **Lance** (`output_format: lance`): Lance **dataset directories** (`shard-000000.lance/` with `data/`, `_versions/`, `_transactions/`) written via `lance.write_dataset` / `LanceFragment` — each shard is a directory tree, not one R2 object. `audio` (float16) and `mel_spec` (float32) are stored as opaque `large_binary` BLOB columns — raw per-row bytes whose inner shape/dtype live in the `synth_setter.blob_field_specs` schema metadata and decode client-side — because the DuckDB/Lance reader pre-allocates a full vector chunk per fixed-shape-tensor column sized by the tensor width, OOMing SmooSense's Table view even on a 4-row shard; `param_array` (float32) stays a fixed-shape tensor (it is tiny). The `ShardMetadata` JSON is embedded in the schema metadata and the on-disk format is pinned to `data_storage_version="2.1"`. Per-column projection still works (stats streaming reads only `mel_spec`); finalize streams each split's shards directly from R2 and writes `train.lance` / `val.lance` / `test.lance` straight back as dataset directories.

...
| **Shard**                  | A single R2 object containing a batch of training samples (audio, mel spectrograms, parameter arrays) in the run's `output_format` — HDF5 file, WebDataset tar, or single-file Lance dataset. Typically 1k-10k samples per shard. Named by logical index plus the format suffix (`shard-000042.h5` / `.tar` / `.lance`).                                                                                                                                                                                                                                                                                                                                                         |

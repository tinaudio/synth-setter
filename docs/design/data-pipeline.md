...
- **Lance** (`output_format: lance`): Lance **dataset directories** (`shard-000000.lance/` with `data/`, `_versions/`, `_transactions/`) written via `lance.write_dataset` / `LanceFragment` — each shard is a directory tree, not one R2 object. Rows are Arrow fixed-shape-tensor columns (float16 `audio`, float32 `mel_spec` / `param_array`) with the `ShardMetadata` JSON embedded in the schema metadata and the on-disk format pinned to `data_storage_version="2.2"`. The columnar layout gives per-column projection (stats streaming reads only `mel_spec`); finalize streams the train shards directly from R2 through Welford for `stats.npz` and leaves every shard in place under the run prefix — no merged `train/val/test.lance` is written (the merge re-shipped the whole dataset through the finalize host). Training consumes the per-shard datasets via `ShardedLanceVSTDataModule`, which resolves each split's shard subset from `input_spec.json`'s `split_shard_ranges`.

...
latent column, both derived from the audio column. A resume cache (on by
default at an identity-keyed path; `--resume-cache` overrides it and
`--no-resume-cache` disables it) stores per-batch encoder outputs so an

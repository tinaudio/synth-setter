...
src/
  ...
  pipeline/             # Distributed-pipeline package
    ...
    skypilot_launch.py  # Click CLI thin passthrough: shells out to an operator-supplied inner generator command, discovers the materialized `input_spec.json` via `find_input_specs`, resolves its canonical R2 URI in-process via `_resolve_spec_uri`, prints it on stdout as a `::synth-setter-spec-uri::<uri>` sentinel for the CI workflow to harvest, and hands off to `dispatch_via_skypilot`.

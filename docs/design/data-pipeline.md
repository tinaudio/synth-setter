...
**Lance** (`output_format: lance`): workers emit one Lance **dataset directory** per shard (`shard-000000.lance/` with `data/`, `_versions/`, `_transactions/`) under the run prefix. Rows are Arrow fixed-shape-tensor columns (float16 `audio`, float32 `mel_spec` / `param_array`) with the `ShardMetadata` JSON embedded in the schema metadata and the on-disk format pinned to `data_storage_version="2.2"`. Whether finalize joins those shards into `train/val/test.lance` depends on the finalize strategy: the **shipped** finalize is stats-only and leaves the per-shard datasets in place (see *Stats-only finalize* below); the **target** fragment-commit design instead persists a strict Pydantic `fragment.json` sidecar per shard (whose `fragment_json` field is the exact Lance `FragmentMetadata.to_json()` payload, with logical identity — `shard_id`, `split`, `worker_id`, `attempt_uuid` — derived from the staging path, filename, and spec rather than stored in the sidecar; [§14.4](#144-lance-fragment-sidecar-schema)) and commits the winning fragments into merged split datasets with no row copy.

**Why Lance is primary:** the columnar layout gives per-column projection (train on `mel_spec` + `param_array` without decoding `audio`), and the dataset streams natively from object storage for both random-access and sequential loaders. One format serves both the local single-GPU case (HDF5's niche) and the multi-GPU streaming case (WDS's niche).

**Stats-only finalize:** `finalize_lance` streams only the train shards from R2 through Welford to compute `stats.npz` and leaves every `shard-*.lance` in place under the run prefix — it writes no merged `train/val/test.lance` (merging them would re-ship the whole dataset through the finalize host, which hit bandwidth limits on large runs). Training consumes the per-shard datasets directly via `ShardedLanceVSTDataModule`, which resolves each split's shard subset from `input_spec.json`'s `split_shard_ranges` and reads them as one logical dataset (typically over an `rclone mount` of the run prefix). A future fragment-metadata-commit finalize could instead join the shards into one manifest with no data copy ([§12](#12-open-questions-risks--limitations)).

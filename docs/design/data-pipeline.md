...
**Browsing Lance shards (SmooSense):** the single-file Lance format the pipeline writes is not what SmooSense's `sense db` opens — that browser reads Lance *dataset* directories. `synth-setter-browse-dataset <shard|split|r2://uri> …` bridges the gap: it re-materializes each source (downloading `r2://` splits first) as a Lance dataset under one browse-db directory, carrying the embedded `ShardMetadata` onto the dataset schema, then launches `sense db` on it. SmooSense is not a project dependency (it needs Python ≥3.11, above this project's ≥3.10 floor); install it once as an isolated tool with `uv tool install -U smoosense`. See [the browsing guide](../guides/browse-lance-datasets.md).

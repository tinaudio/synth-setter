...
### Authoring the sketch instead of extracting it

"Sketch source" switches the page from the sketch extracted from the upload to one generated by
the controls in the "Authored reverb sketch" panel. The generator evaluates closed forms on the
extractor's log-time grid:

| Control                              | Range                                     | Row it generates                                              |
| ------------------------------------ | ----------------------------------------- | ------------------------------------------------------------- |
| RT60 per octave band (advanced)      | 0.1 to 4 s, the parameter spec's RT range | rows 0 to 7 as `-60 t / RT60` dB, clipped to -60 dB           |
| RT60 tilt, low and high bands        | 0.25 to 4 multipliers                     | scales the four low or four high band RT60s together          |
| Pre-delay                            | 0 to 100 ms                               | holds every decay row at full energy until it elapses         |
| Mixing time and initial echo density | 0 to 200 ms, 0 to 1                       | row 8 ramps from the initial density to the diffuse reference |
| Spectral flatness, start and end     | 0 to 1 each                               | row 9 as a linear ramp across the 32 intervals                |

The heatmap under the form always shows the 10 × 32 grid the model will receive. The upload still
supplies the mel content, so compare an authored sketch on its own by choosing the sketch-only
conditioning mode. The checkpoint trained on sketches extracted from its own renders, so decays
no 8-line FDN can produce give unpredictable parameters.

...
(`DatasetSpec.r2_bucket`) — flows in through the materialized spec at
`--spec`.
...
file passed via `--spec`. `input_spec.json` is written by the caller (a
Hydra composition over `configs/dataset.yaml` produces a `DatasetSpec`
that the entrypoint serializes via `model_dump_json()`) to a bind-mounted
...
from `DatasetSpec.r2_bucket` and is interpolated into upload paths by
`generate_dataset.py` (`r2:${spec.r2_bucket}/...`).

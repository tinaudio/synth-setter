...
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- |
| Hydra-config validation                           | [`tests/test_configs.py`](../../tests/test_configs.py)                                                                                                                         | Uses `cfg_train` / `cfg_eval` / `cfg_dataset` fixtures; light assertions + `hydra.utils.instantiate()` / `spec_from_cfg`                                                                                                                                                                         | (none by default)                                             |
...
| E2E dataset-generation entrypoint                 | [`tests/test_generate_dataset.py`](../../tests/test_generate_dataset.py)                                                                                                       | Uses `cfg_dataset` fixture; drives `from_hydra(cfg_dataset)` or CLI subprocess; no Hydra config-initializer imports (enforced by `tests/_meta/test_entrypoint_e2e_only.py`)                                                                                                                      | `integration_r2`, `r2`, `requires_vst`, `slow`                |
...
| Docker image smoke tests                          | [`tests/docker/test_smoke.py`](../../tests/docker/test_smoke.py)                                                                                                               | Runs inside the built container image                                                                                                                                                                                                                                                            | `docker_smoke` (+ `requires_vst`, `slow` on per-synth loads)  |
| VST integration tests                             | [`tests/data/vst/test_preset_params.py`](../../tests/data/vst/test_preset_params.py), [`tests/data/vst/test_cardinal_preset.py`](../../tests/data/vst/test_cardinal_preset.py) | Requires the matching VST3 binary on disk; Cardinal tests its committed Rack patch and mapped host controls                                                                                                                                                                                      | `requires_vst`                                                |
| Devcontainer + GHA invariants                     | [`tests/infra/`](../../tests/infra) — has its own [`conftest.py`](../../tests/infra/conftest.py)                                                                               | Static checks on `.devcontainer/`, `Dockerfile`, workflow YAML, and Makefile↔Dockerfile plugin-pin parity; opt-in subprocess timing and sandboxed `make` runs; PR-review model-routing + review-launcher contract tests (Claude/Codex/OpenCode role pinning, launcher dry-run and degrade paths) | `infra`                                                       |
...
| Parallel real-Surge render (cadence)              | [`tests/integration/test_parallel_shard_render_linux.py`](../../tests/integration/test_parallel_shard_render_linux.py)                                                         | Real Surge XT renders under the Xvfb wrapper, parametrized over cadence cells; runs in `test-vst-slow.yml`                                                                                                                                                                                       | `requires_vst`, `slow`                                        |
| Lance dataloader streaming                        | [`tests/data/test_lance_torch.py`](../../tests/data/test_lance_torch.py), [`tests/integration/test_lance_torch_r2.py`](../../tests/integration/test_lance_torch_r2.py)         | Local e2e + real-R2 streaming through the `lance.torch` dataloaders; datasets written by the real pipeline writer, no fakes or mocks                                                                                                                                                             | `integration_r2`, `r2`, `slow` (R2 suite only)                |
| Structural invariants (meta-tests)                | [`tests/_meta/`](../../tests/_meta)                                                                                                                                            | AST-based static checks that enforce architectural rules on other test modules; no fixtures, no markers, no heavy imports — run in the default fast suite                                                                                                                                        | (none)                                                        |
| Test helpers                                      | [`tests/helpers/`](../../tests/helpers)                                                                                                                                        | Not tests themselves — import from `tests.helpers.<name>`                                                                                                                                                                                                                                        | —                                                             |

Multiple conftests live under `tests/`; pytest resolves parent conftests, but each subtree carries its own fixtures: [`tests/conftest.py`](../../tests/conftest.py) (Hydra `cfg_train` / `cfg_train_lance` / `cfg_eval` / `cfg_dataset` / `cfg_surge_xt`), [`tests/pipeline/conftest.py`](../../tests/pipeline/conftest.py) (pipeline-specific), [`tests/infra/conftest.py`](../../tests/infra/conftest.py) (intentionally minimal — only stdlib imports), [`tests/integration/conftest.py`](../../tests/integration/conftest.py) (thin re-export of `fake_r2_remote` from `tests/pipeline/conftest.py`), and [`tests/claude_hooks/conftest.py`](../../tests/claude_hooks/conftest.py) (autouse scrub of the `GATE_OVERRIDE_ENV_VARS` gate overrides so hook subprocesses see their documented default modes; tests exercising a non-default mode opt back in via their subprocess `env`). The infra suite's own conftest pulls in no heavy deps, but pytest still walks up to the top-level `tests/conftest.py` (torch/Hydra) during normal collection. To skip the parent and keep the infra suite stdlib-only, run [`make test-infra`](../../Makefile), which invokes `pytest tests/infra/ --confcutdir=tests/infra`.
...
- [`cpu-slow.yml`](../../.github/workflows/cpu-slow.yml) — slow CPU-only suite (runs `make test-ci-slow`), post-merge on `main`. **Post-merge by design** so PR feedback isn't gated on the long suite, which runs serially (no `-n auto`). The lane runs on the standard `ubuntu-latest` runner; the workflow's comment records the `ubuntu-latest-4core` fallback for exit-137 OOMs. PR-time coverage of these tests comes from `test-mps.yml`. VST-marked tests live in [`test-vst-slow.yml`](../../.github/workflows/test-vst-slow.yml), whose per-synth Docker matrix includes Cardinal's mapped-control contract. On post-merge failure, this workflow auto-opens a `ci-automation` Bug ticket to `@ktinubu` (deduped by title).
...
- [`nightly.yml`](../../.github/workflows/nightly.yml) — scheduled full `pytest` run on CPU (`ubuntu-latest`) via `make test-ci-nightly`; excludes `gpu`/`mps`/`requires_vst` markers (no CUDA/MPS device and no Surge XT VST3 on the runner — those suites run on dev-snapshot via sibling workflows, see [#1279](https://github.com/tinaudio/synth-setter/issues/1279)).

**Coverage strategy in one sentence:** `[cpu]` parametrizations of slow tests run post-merge on the standard `ubuntu-latest` runner (`cpu-slow.yml`); `[mps]` parametrizations run pre-submit on the macOS runner (`test-mps.yml`); `[gpu]` parametrizations run twice-weekly on the GPU runner (`test-gpu.yml`). A regression in the cpu path is caught after merge, not before — accepted because the cost of running CPU PyTorch on every PR is OOM failures, not just minutes.
...
| Assert a dataset Hydra config composes and validates                             | `tests/test_generate_dataset.py::test_cfg_dataset_composes_and_validates_as_dataset_spec` | `cfg_dataset`                                                                          | —                                              |
| E2E `from_hydra(cfg_dataset)` shard render → real R2                             | `tests/test_generate_dataset.py::test_generate_dataset_renders_shards_to_r2`              | `cfg_dataset`, `monkeypatch`                                                           | `integration_r2`, `r2`, `requires_vst`, `slow` |
| E2E `generate-dataset` CLI subprocess → assert inline-oracle-eval `metrics.json` | `tests/test_generate_dataset.py::test_oracle_eval_inline_writes_bounded_audio_metrics`    | `cfg_dataset`, `tmp_path`                                                              | `integration_r2`, `r2`, `requires_vst`, `slow` |
...
Most categories don't need the `cfg_train` / `cfg_train_lance` / `cfg_eval` / `cfg_dataset` / `cfg_surge_xt` fixtures. They're specifically for Hydra-composed code paths.

______________________________________________________________________

## 4. Hydra cfg fixtures (`tests/conftest.py`)

Only needed for tests that exercise Hydra-composed configs (test_configs, test_train, test_eval, test_benchmarks, test_generate_dataset, etc.).
...
A fourth fixture group — `cfg_dataset_global` / `cfg_dataset` — covers the `synth-setter-generate-dataset` CLI (`tests/test_generate_dataset.py`). It composes `dataset.yaml` with `experiment=generate_dataset/smoke-shard`, then redirects `paths.{output_dir,work_dir,log_dir}` into `tmp_path`. Unlike its siblings it **omits** `return_hydra_config=True` because the dataset cfg's `hydra.sweep.subdir` interpolates `${hydra.job.num}` (resolved only at runtime by `@hydra.main`); leaking the `hydra.*` sub-tree into the fixture would break `spec_from_cfg`'s `resolve=True` round-trip. The e2e tests use uuid-suffixed R2 prefixes and real renderer subprocesses against Cloudflare R2; `cfg_dataset_cardinal` composes the three-worker `cardinal-smoke` experiment, finalizes its real Lance fragments, and reads every artifact and tensor column back. They are gated by `[integration_r2, r2, requires_vst, slow]`, auto-skip when R2 is unreachable, and run in [`test-generate-dataset-shards.yml`](../../.github/workflows/test-generate-dataset-shards.yml), which provides Surge XT and Cardinal Synth inside Docker.

...
| `eval-{config_id}`       | `build_eval_results_artifact` / `_log_eval_results_artifact` in `src/synth_setter/cli/eval.py`   | After the UUID-scoped eval output dir is uploaded immutably to R2 (global-zero only); type `eval-results`, exact attempt `s3://` R2 reference (`checksum=False`), metadata = scalar summary metrics + `git_sha`                                                                                                                          |

...
| `LogPerParamMSE`         | `per_param_mse/{name}`, `val/param_mse_spec_quantized`, `per_param_mse_spec_quantized/{name}`, plus optional swap metrics | `on_validation_epoch_end` (via `self.log_dict`)                                                                                                                   | `src/synth_setter/utils/callbacks.py::LogPerParamMSE`                                                                |
...
`val/param_mse_spec_quantized` clips predictions to the ParamSpec domain and
snaps categorical and integral fields to the values used for rendering before
scoring them against the targets. `ValAudioProbe`'s keys mirror §2i's `audio/*`

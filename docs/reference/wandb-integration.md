...
| Console capture   | `wandb.Settings(console="wrap", console_multipart=True)` — `redirect` captures into a local `output.log` that wandb 0.26.x never uploads (empty UI Logs tab); `wrap` is the only mode that reaches the server but sees only this process's Python-level writes, so `generate_dataset` tees render/rclone/eval subprocess output through `sys.stderr` (`check_call_streamed` in `src/synth_setter/pipeline/subprocess_stream.py`, exit-keyed so a pipe-holding descendant can't stall it). Multipart gives each resumed session (generate → finalize → oracle eval) its own `logs/output_*.log` instead of overwriting one `output.log` | `src/synth_setter/configs/logger/wandb.yaml` § `wandb.settings` |

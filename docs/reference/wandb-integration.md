...
| `eval-{config_id}`       | `build_eval_results_artifact` / `_log_eval_results_artifact` in `src/synth_setter/cli/eval.py`   | After an attempt-owned payload is uploaded and its suite-root `latest.json` pointer is committed (global-zero only); type `eval-results`, immutable attempt `s3://` R2 reference (`checksum=False`), metadata = scalar summary metrics + `git_sha`                                                                                       |

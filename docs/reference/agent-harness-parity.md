...
| `pre-pr-review-gate.sh` (registration suspended)   | Nothing locally while #2020 is unresolved                    | Not registered                         | **Strong.** The retained hook only front-loads findings locally. The active merge gates are `test.yml` (unit suite), `code-quality-pr.yaml` (pre-commit), `pr-metadata-gate.yaml` (linked issue + title), and Copilot review.                                                                                         |
| `pr-readiness-stop.sh` (Stop)                      | Ending a turn while readiness is `WAIT` or `ACTION_REQUIRED` | `PR_READINESS_GATE` (default `block`)  | **Strong.** Branch protection + required status checks + Copilot review gate the merge regardless of whether the local Stop hook fired; `warn` mode loses only the turn-level nag, not merge safety.                                                                                                                  |

...
| Pipeline input + runtime spec | Pydantic `BaseModel(frozen=True, extra="forbid")` — `DatasetSpec` unifies the prior config + materialized-spec split. Sub-models (`RenderConfig`, `ShardSpec`) are strict; the top-level model is intentionally non-strict so JSON round-trips coerce `list→tuple` / `str→datetime`. | Parse-time — Hydra `compose` → `spec_from_cfg()` (#887, #912, #917 unified the prior `DatasetConfig` + `DatasetPipelineSpec` split into one model that is both the validated input *and* the materialized artifact on R2) | git (`configs/experiment/`) for input; R2 (`{r2_prefix}/input_spec.json`) for the serialized JSON | `configs/experiment/surge-simple-480k-10k.yaml` |
...
- Invoked via: `python -m pipeline.entrypoints.skypilot_launch --experiment <name> --template <yaml>` (trailing positional args, e.g. `render.plugin_path=...`, are forwarded to Hydra `compose` as overrides).

...
- `render=pyfdn synth=pyfdn_n8_mono` selects the native pyFDN backend. It renders four-second, 44.1 kHz mono impulse responses by default and samples complete 91-coordinate patches through the shared `AudioRenderer` acceptance loop. Set `render.pyfdn_excitation=chirp` to use the canonical in-process chirp instead. MIDI fields are fixed compatibility stubs; R2 stores only generated dataset outputs.

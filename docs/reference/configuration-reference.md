...
- `experiment=pyfdn/flow_audio_flamo` and `experiment=pyfdn/ffn_audio_flamo` inject a FLAMO differentiable renderer into audio feedback; dataset generation and evaluation still use pyFDN. Flow renders its one-step endpoint estimate with time-dependent weighting, while the FFN renders its direct prediction at full weight. The renderer factory adapts the fixed Householder, Householder-vector and Kronecker `BasicFDNParamSpec` identities at 44.1 kHz; the general renderer accepts complete multichannel `BasicFDN` builds and preserves every input/output transfer path. Gains, continuous feedback coordinates and DC/Nyquist RT controls are differentiable; integer delays and reflection choices have zero gradient. See [pyFDN audio feedback](../experiments/pyfdn-flow-audio.md) for the channel contract and advanced-effect exclusions. The FFT period defaults to the next power of two above twice the output length; increase `model.audio_loss.renderer.fft_size` to reduce circular tail aliasing. Single-device and `model.compile=false` restrictions remain. Real local generation → training → checkpoint reload → evaluation tests run in the existing PR CPU-slow CI job without R2 credentials.

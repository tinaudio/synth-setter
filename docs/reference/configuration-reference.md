...
- `render=pyfdn synth=pyfdn_n8_mono` selects the native pyFDN backend. It generates the canonical 44.1 kHz mono chirp in each renderer process and samples complete 91-coordinate patches through the shared `AudioRenderer` acceptance loop. MIDI fields are fixed compatibility stubs; R2 stores only generated dataset outputs.

...
| `materialization_profile`              | string         | `safe` uses Lance's memory defaults; `high_throughput` enables the high-memory scanner and writer tuning used by full-data RunPod launches                                              | `src/synth_setter/pipeline/data/lance_materialize.py`     |

...
- `experiment=pyfdn/flow_audio_flamo` injects a FLAMO differentiable renderer into audio feedback; dataset generation and evaluation still use pyFDN. It supports only `pyfdn_n8_mono_householder` mono impulse responses at 44.1 kHz. B/C/D and DC/Nyquist RT controls are differentiable; integer delays have zero gradient. The FFT period defaults to the next power of two above twice the output length; increase `model.audio_loss.renderer.fft_size` to reduce circular tail aliasing. Single-device and `model.compile=false` restrictions remain. The real local generation → training → checkpoint reload → evaluation test runs in the existing PR CPU-slow CI job without R2 credentials.

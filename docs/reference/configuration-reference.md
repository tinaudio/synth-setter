...
| Pipeline input + runtime spec | Pydantic `BaseModel(frozen=True)` (`DatasetSpec`) | Parse-time — Hydra `compose` → `spec_from_cfg()` (#887, #912, #917 unified the prior `DatasetConfig` + `DatasetPipelineSpec` split into one model that is both the validated input *and* the materialized artifact on R2) | git (`configs/experiment/`) for input; R2 (`{r2_prefix}/input_spec.json`) for the serialized JSON | `configs/experiment/surge-simple-480k-10k.yaml` |

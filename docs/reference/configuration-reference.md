...
| Pipeline input + runtime spec | Pydantic `BaseModel(frozen=True, extra="forbid")` — `DatasetSpec` unifies the prior config + materialized-spec split. Sub-models (`RenderConfig`, `ShardSpec`) are strict; the top-level model is intentionally non-strict so JSON round-trips coerce `list→tuple` / `str→datetime`. | Construction-time — `DatasetSpec(**spec_dict)` (current path: `load_dataset_spec_yaml()`; Hydra compose lands in PR-3 / #917) | git (`configs/dataset/`, migrating to `configs/experiment/`) → R2 (`{r2_prefix}/input_spec.json`) | `configs/dataset/surge-simple-480k-10k.yaml` |

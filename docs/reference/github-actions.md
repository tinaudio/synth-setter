...
| `test-oci-image-bake`       | PR-triggered gate: calls the reusable `oci-image-bake.yaml` with a PR-scoped `image_tag` and verifies the produced image is pullable.                                                                                                                                                                                                                                                                                                                                                                                           | Inert until #921 lands `oci-image-bake.yaml`; path filter currently matches no PRs. Uses `pr-<PR#>` tags so production tags (`dev-snapshot`, etc.) are never overwritten.                                              |

...
| `test-expensive`          | Runs `slow`-marked pytest (excluding `gpu` and `mps`) on a larger ubuntu runner, post-merge on `main`. | Skips docs-only merges (`paths-ignore`). Concurrency-grouped to queue overlapping runs. See [Concurrency](#concurrency).                       |

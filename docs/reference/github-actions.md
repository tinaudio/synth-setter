...
| `R2_ACCESS_KEY_ID`         | `dataset-generation`, `spec-materialization`, `test-dataset-generation`, `test-skypilot-debug` | Cloudflare R2 credentials. Bare-named in GitHub Secrets; the launcher's cred-bootstrap (`scripts/skypilot_write_provider_creds.sh`) bridges them to `RCLONE_CONFIG_R2_*` for rclone. |
...
| `R2_ACCOUNT_ID`            | `test-dataset-generation`, `test-skypilot-debug`                                               | Cloudflare account ID; cred-bootstrap writes it to `~/.cloudflare/accountid` for SkyPilot's R2 storage adaptor.                                                                      |
| `RUNPOD_API_KEY`           | `test-dataset-generation` (`runpod` cells only), `test-skypilot-debug`                         | RunPod API token; written to `~/.runpod/config.toml` so SkyPilot can provision pods on demand.                                                                                       |
...
| `OCI_COMPARTMENT_OCID`     | `test-dataset-generation` (`oci` cells only)                                                   | OCI compartment OCID (root or child); written to `~/.sky/config.yaml` so SkyPilot launches target it.                                                                                |

...
| Workflow                    | Purpose                                                                                                                                                                                                                                                                                                                                                                                 | Gotcha                                                                                                                                                                                                                 |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `generate-dataset-shards`   | Reusable workflow: invokes `pipeline.entrypoints.skypilot_launch` for one provider (skypilot-local kind / runpod / oci); the launcher uploads the materialized spec to R2 and the workflow emits a `spec_uri` output that downstream callers (`validate-dataset-shards`) consume. A run-metadata artifact is also uploaded for debugging. Consumed by `test-dataset-generation`.        | Mounts PR code as a volume into the container for runpod/oci rows. See [Mount-as-volume pattern](#mount-as-volume-pattern).                                                                                            |
| `validate-dataset-shards`   | Reusable workflow: structural spec validation (`pipeline.ci.validate_spec`) + per-shard HDF5 validation (`pipeline.ci.validate_shard`) against shards downloaded from R2. Consumes the `spec_uri` (`r2://`) workflow output emitted by `generate-dataset-shards` (or any caller that uploaded a spec to a known r2:// URI). All rclone copies happen inside `pipeline.r2_io`.           |                                                                                                                                                                                                                        |

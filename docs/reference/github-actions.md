...
| `RCLONE_CONFIG_R2_ACCESS_KEY_ID`          | `dataset-generation`, `spec-materialization`, `test-dataset-generation`, `test-skypilot-debug`, `test-skypilot-local` | Cloudflare R2 credentials passed as runtime env vars to `docker run` (rclone reads `RCLONE_CONFIG_R2_*` directly).                                                                                                                                                                                                                                                      |
| `RCLONE_CONFIG_R2_SECRET_ACCESS_KEY`      | same as above                                                                                                         | Paired with `RCLONE_CONFIG_R2_ACCESS_KEY_ID`.                                                                                                                                                                                                                                                                                                                           |
| `RCLONE_CONFIG_R2_ENDPOINT`               | `dataset-generation`, `spec-materialization`, `test-dataset-generation`, `test-skypilot-debug`, `test-skypilot-local` | R2 endpoint URL (runtime).                                                                                                                                                                                                                                                                                                                                              |
| `R2_ACCOUNT_ID`                           | `test-dataset-generation`, `test-skypilot-debug`                                                                      | Cloudflare account ID; written to `~/.cloudflare/accountid` by `scripts/skypilot_write_provider_creds.sh` for SkyPilot's R2 storage adaptor (#749). Required by every launcher invocation (`skypilot-local`, `runpod`, `oci`). No rclone-prefixed alias — must be set under this name.                                                                                  |

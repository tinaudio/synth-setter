...
| `R2_ACCOUNT_ID`                           | `test-dataset-generation`, `test-skypilot-debug`, `check-auth`                                                                                                      | Cloudflare account ID; written to `~/.cloudflare/accountid` by `scripts/skypilot_write_provider_creds.sh` for SkyPilot's R2 storage adaptor (#749). Required by the `runpod` and `oci` launcher invocations; the `skypilot-local` row no longer triggers the cred bootstrap that writes `~/.cloudflare/accountid`. No rclone-prefixed alias — must be set under this name. |

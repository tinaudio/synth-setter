...
| `test-skypilot-oci-debug`   | OCI sibling of `test-skypilot-debug`'s noop variant. Provisions an OCI VM via SkyPilot's OCI backend (shape per `configs/compute/oci-debug-template.yaml`), runs an inline noop probe, tears down. Triggers on every PR touching the template or the workflow itself, plus `workflow_dispatch`. | Concurrency-grouped per-ref to avoid stacked runs fighting over OCI tenancy quota. Forks skipped (no secrets).                                                                                   |

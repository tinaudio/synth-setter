...
| `test-dataset-generation`   | Dynamic matrix over `pipeline.entrypoints.skypilot_launch_smoke`. Same-repo PR events run **OCI-only**; fork PRs run an empty matrix (no secrets); `workflow_dispatch` selects via `inputs.provider` (`all` / `runpod` / `oci`). Validates the resulting spec and shard per provider. | Provisions a real RunPod pod and/or OCI VM (both billable). Needs `RUNPOD_API_KEY` + the six `OCI_*` secrets in addition to the `R2_*` secrets. |

...
| `test-dataset-generation`   | Dynamic matrix over the data-pipeline smoke. Same-repo PR events run **`skypilot-local`-only** (kind cluster via `sky local up` + the unified launcher against `local-template.yaml` — no billable capacity); fork PRs run an empty matrix (no secrets); `workflow_dispatch` selects via `inputs.provider` (`all` = `skypilot-local` + `runpod` + `oci`; or `local` / `skypilot-local` / `runpod` / `oci`). The legacy `local` row (in-runner container, no SkyPilot) stays as a manual-dispatch fallback. Validates the resulting spec and shard per provider. | Only `runpod` / `oci` matrix cells provision billable capacity (a RunPod pod and/or OCI VM) and need `RUNPOD_API_KEY` + the six `OCI_*` secrets; `skypilot-local` runs on the GH runner against a local kind cluster (no cloud creds — only R2). |
...
| `check-auth`                | `workflow_dispatch` + weekly cron. Matrix over `runpod` / `oci` / `local` provider modes of `scripts/skypilot_write_provider_creds.sh`; validates the file shape (mode 600), the structural rclone-env emission, R2 auth (`rclone lsd r2:`), and provider auth (`sky check`). Skips when `SKYPILOT_API_SERVER_ENDPOINT` is set.                                                                                                                                                                                                                                 | The `local` leg skips `sky check` (kind has no compute provider to validate). Run after rotating any provider token to confirm the bootstrap can still write the cred files end-to-end.                                                          |
...
inside the same image on the GH runner; no SkyPilot.)
...
- `test-dataset-generation` writes spec + run log to a per-provider artifact named `test-run-metadata-<provider>` for each matrix cell; the `validate-spec` and `validate-shard` matrix jobs in the same workflow consume the artifact for their cell's provider (single-workflow chain, no cross-workflow handoff).
...
| `R2_ACCESS_KEY_ID`                        | `dataset-generation`, `spec-materialization`, `test-dataset-generation`, `test-skypilot-debug`, `test-skypilot-local`, `check-auth` | Cloudflare R2 credentials. Bare-named in GitHub Secrets; the launcher's cred-bootstrap (`scripts/skypilot_write_provider_creds.sh`) bridges them to `RCLONE_CONFIG_R2_*` for rclone.                                                                                                                                                                                    |
| `R2_SECRET_ACCESS_KEY`                    | same as above                                                                                                                       | Paired with `R2_ACCESS_KEY_ID`.                                                                                                                                                                                                                                                                                                                                         |
| `R2_ENDPOINT`                             | `dataset-generation`, `spec-materialization`, `test-dataset-generation`, `test-skypilot-debug`, `test-skypilot-local`, `check-auth` | R2 endpoint URL (runtime).                                                                                                                                                                                                                                                                                                                                              |
| `R2_ACCOUNT_ID`                           | `test-dataset-generation`, `test-skypilot-debug`, `check-auth`                                                                      | Cloudflare account ID; cred-bootstrap writes it to `~/.cloudflare/accountid` for SkyPilot's R2 storage adaptor.                                                                                                                                                                                                                                                         |
| `RUNPOD_API_KEY`                          | `test-dataset-generation` (`runpod` cells only), `test-skypilot-debug`, `check-auth` (`runpod` leg)                                 | RunPod API token; written to `~/.runpod/config.toml` by `scripts/skypilot_write_provider_creds.sh` so SkyPilot can provision pods on demand. Skipped when `SKYPILOT_API_SERVER_ENDPOINT` is set — the remote API server holds provider creds (#785).                                                                                                                    |
| `SKYPILOT_API_SERVER_ENDPOINT` (optional) | `test-dataset-generation`, `test-skypilot-debug`, `check-auth`                                                                      | URL of a remote SkyPilot API server with HTTP basic auth embedded. When set, all `sky.*` calls dispatch to that server (which has its own provider creds), so the local cred bootstrap (`scripts/skypilot_write_provider_creds.sh`) is skipped — see [`docs/operations/skypilot-remote-server-playbook.md`](../operations/skypilot-remote-server-playbook.md) and #785. |
| `OCI_USER_OCID`                           | `test-dataset-generation` (`oci` cells only), `check-auth` (`oci` leg)                                                              | OCI user OCID (Identity → Domains → Users); written to `~/.oci/config`.                                                                                                                                                                                                                                                                                                 |
| `OCI_TENANCY_OCID`                        | `test-dataset-generation` (`oci` cells only), `check-auth` (`oci` leg)                                                              | OCI tenancy OCID (root account identifier); written to `~/.oci/config`.                                                                                                                                                                                                                                                                                                 |
| `OCI_FINGERPRINT`                         | `test-dataset-generation` (`oci` cells only), `check-auth` (`oci` leg)                                                              | API signing key fingerprint paired with `OCI_API_KEY_PEM`; written to `~/.oci/config`.                                                                                                                                                                                                                                                                                  |
| `OCI_REGION`                              | `test-dataset-generation` (`oci` cells only), `check-auth` (`oci` leg)                                                              | OCI region identifier (e.g. `us-ashburn-1`); written to `~/.oci/config`.                                                                                                                                                                                                                                                                                                |
| `OCI_COMPARTMENT_OCID`                    | `test-dataset-generation` (`oci` cells only), `check-auth` (`oci` leg)                                                              | OCI compartment OCID (root or child); written to `~/.sky/config.yaml` so SkyPilot launches target it.                                                                                                                                                                                                                                                                   |
| `OCI_API_KEY_PEM`                         | `test-dataset-generation` (`oci` cells only), `check-auth` (`oci` leg)                                                              | Full PEM of the API signing private key; written to `~/.oci/oci_api_key.pem`.                                                                                                                                                                                                                                                                                           |

...
| `test-expensive`          | Runs `slow`-marked pytest (excluding `gpu`) on `ubuntu-latest`, post-merge on `main`.           | Static MNIST cache key (`mnist-dataset-v1`); shared with `test`. See [Caching](#caching).                                                      |

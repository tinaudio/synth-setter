...
| `test-oci-image-bake`       | PR-triggered gate: dispatches `oci-image-bake.yaml` on the PR branch with a PR-scoped image display name, waits for the bake run to finish, then verifies the resulting OCI custom image is registered in the target compartment via `oci compute image list`. Verify job's cleanup step runs under `always()` and re-resolves the OCID by display-name so a leaked image is reaped even if the in-step lookup itself failed.                                                                                                  | `paths:` includes this workflow itself, `oci-image-bake.yaml`, and `configs/compute/oci-cpu-template.yaml`, so edits to any of those trigger the gate. Image display name embeds `pr-<PR#>-<short-sha>` so each commit's bake produces a distinct image and verify can't pass on a stale one. Cleanup is best-effort — the trigger-bake job cancels the dispatched bake on its own cancel/failure to limit the orphan window. Each run dispatches a real bake — billable OCI VM time (~30–90 min) per run. Fork PRs skipped (no OCI secrets). |

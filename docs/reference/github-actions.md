...
| `test-skypilot-oci-debug`   | OCI sibling of `test-skypilot-debug`'s noop variant. Provisions an Always-Free `VM.Standard.E2.1.Micro` via SkyPilot's OCI backend, runs an inline noop probe, tears down. Triggers on every PR touching `configs/compute/oci-debug-template.yaml` or the workflow itself, plus `workflow_dispatch`. | OCI Always-Free quota is one E2.1.Micro per tenancy — concurrency-grouped per-ref to avoid stacked runs fighting over the shape. Forks skipped (no secrets).                                     |
...
| `OCI_USER_OCID`            | `test-dataset-generation`, `test-skypilot-oci-debug`                                           | OCI user OCID (Identity → Domains → Users); written to `~/.oci/config`.                                           |
| `OCI_TENANCY_OCID`         | `test-dataset-generation`, `test-skypilot-oci-debug`                                           | OCI tenancy OCID (root account identifier); written to `~/.oci/config`.                                           |
| `OCI_FINGERPRINT`          | `test-dataset-generation`, `test-skypilot-oci-debug`                                           | API signing key fingerprint paired with `OCI_API_KEY_PEM`; written to `~/.oci/config`.                            |
| `OCI_REGION`               | `test-dataset-generation`, `test-skypilot-oci-debug`                                           | OCI region identifier (e.g. `us-ashburn-1`); written to `~/.oci/config`.                                          |
| `OCI_COMPARTMENT_OCID`     | `test-dataset-generation`, `test-skypilot-oci-debug`                                           | OCI compartment OCID (root or child); written to `~/.sky/config.yaml` so SkyPilot launches target it.             |
| `OCI_API_KEY_PEM`          | `test-dataset-generation`, `test-skypilot-oci-debug`                                           | Full PEM of the API signing private key; written to `~/.oci/oci_api_key.pem`.                                     |

...
| `validate-dataset-shards` | Reusable + dispatchable workflow: structural spec validation (`src.pipeline.ci.validate_spec`) + per-shard validation (`src.pipeline.ci.validate_shard`, dispatching by suffix on HDF5 `.h5` and WDS `.tar`) against shards downloaded from R2. Consumes the `spec_uri` (`r2://`) workflow output emitted by `generate-dataset-shards` (or any caller that uploaded a spec to a known r2:// URI). All rclone copies happen inside `src.pipeline.r2_io`. Also dispatchable directly with a hand-supplied `spec_uri`.             |                                                                                                                                                                                                                        |
...
| `RCLONE_CONFIG_R2_ACCESS_KEY_ID`          | `generate-dataset-shards`, `validate-dataset-shards`, `test-dataset-generation`, `test-skypilot-debug`, `test-skypilot-local`, `check-auth` | Cloudflare R2 credentials passed as runtime env vars to `docker run` (rclone reads `RCLONE_CONFIG_R2_*` directly).                                                                                                                                                                                                                                                      |
| `RCLONE_CONFIG_R2_SECRET_ACCESS_KEY`      | same as above                                                                                                                               | Paired with `RCLONE_CONFIG_R2_ACCESS_KEY_ID`.                                                                                                                                                                                                                                                                                                                           |
| `RCLONE_CONFIG_R2_ENDPOINT`               | `generate-dataset-shards`, `validate-dataset-shards`, `test-dataset-generation`, `test-skypilot-debug`, `test-skypilot-local`, `check-auth` | R2 endpoint URL (runtime).                                                                                                                                                                                                                                                                                                                                              |
...
`docker-build-validation` publishes `tinaudio/synth-setter` as a public image. The image contains no baked credentials and the build uses no BuildKit secrets — the public repo is fetched anonymously. R2 + W&B credentials and the target R2 bucket name are passed in at runtime as env vars (`RCLONE_CONFIG_R2_*`, `WANDB_API_KEY`, `R2_BUCKET`). Pipeline workflows (`generate-dataset-shards`, `test-dataset-generation`) pull the image anonymously and pipe credentials via `docker run --env-file` or `-e`.

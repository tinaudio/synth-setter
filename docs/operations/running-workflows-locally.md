...
cover `ubuntu-latest`, `ubuntu-latest-4core` (used by
...
| `test-gpu.yml` runs its orchestrator on `ubuntu-latest`, but dispatches a billable RunPod worker through SkyPilot.                                                                | Validate locally with `tests/infra/test_gpu_workflow.py`; dispatch the real workflow only after the RunPod balance preflight and explicit authorization.                        |

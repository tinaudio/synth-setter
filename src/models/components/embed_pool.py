...
def make_sin_pos_enc(max_len, d_enc):  # noqa: ANN001
    pe = torch.zeros(max_len, d_enc)  # pyright: ignore[reportPrivateImportUsage]
    position = torch.arange(0, max_len, dtype=torch.float32).unsqueeze(1)  # pyright: ignore[reportPrivateImportUsage]
    div_term = torch.exp(  # pyright: ignore[reportPrivateImportUsage]
        torch.arange(0, d_enc, 2, dtype=torch.float32) * (-math.log(10000.0) / d_enc)  # pyright: ignore[reportPrivateImportUsage]
    )
    pe[:, 0::2] = torch.sin(position * div_term)  # pyright: ignore[reportPrivateImportUsage]
    pe[:, 1::2] = torch.cos(position * div_term)  # pyright: ignore[reportPrivateImportUsage]

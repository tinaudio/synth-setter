...
async function fakeBundle(root, name, files) {
  ...
  for (const [key, filename] of Object.entries(files)) {
    ...
    manifest.files[key] = { path: filename, sha256: "0".repeat(64) };
  }
  await writeFile(path.join(directory, "manifest.json"), JSON.stringify({ ...manifest, param_spec_name: "pyfdn_n8_mono_householder" }));
  return directory;
}
...
test("exported site contains the page, ports, runtimes, and both bundles", async () => {
  ...
  const modelDirectory = await fakeBundle(root, "model", { frontend: "frontend.onnx", conditioning: "conditioning.onnx", velocity: "velocity.onnx" });
...
test("export refuses a model bundle whose manifest lacks a graph", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "fdn-site-"));
  const modelDirectory = await fakeBundle(root, "model", { frontend: "frontend.onnx", velocity: "velocity.onnx" });
  const faustDirectory = await fakeBundle(root, "faust", { dsp: "dsp.wasm" });

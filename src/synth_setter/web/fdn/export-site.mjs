...
const PAGE_ASSETS = ["index.html", "app.mjs", "audio.mjs", "bundle.mjs", "decode.mjs", "dsp.mjs", "fft.mjs", "inference.mjs", "metrics.mjs", "noise.mjs", "patch.mjs", "render.mjs", "sketch.mjs", "wav.mjs", "weights.mjs"];
const MODEL_FILES = ["frontend", "conditioning", "velocity"];
...
async function requireManifestFiles(directory, label, expectedKeys) {
  ...
  for (const key of expectedKeys) {
    if (!manifest.files?.[key]?.path) throw new Error(`${label} manifest lacks files.${key}`);
    await stat(path.join(directory, manifest.files[key].path));
  }
  return manifest;
}
...
export async function exportSite({ modelDirectory, faustDirectory, outputDirectory, ortDirectory }) {
  ...
  await cp(path.join(webDirectory, "rk4.mjs"), path.join(outputDirectory, "rk4.mjs"));
  await writeFile(
  ...
  return { pageUrl: "fdn/index.html", paramSpecName: modelManifest.param_spec_name };
}

"""MATPAC++ audio-encoder integration through TinyMU's package API.
...
MATPAC_PLUS_FRONTEND = _MatpacPlusFrontendConfig(
    sample_rate=16_000,
    n_fft=400,
    hop_length=160,
    patch_size=16,
...
type MatpacPlusEncodeFn = Callable[[np.ndarray, int], np.ndarray]
...
def _verified_checkpoint(path: Path) -> Path:
    ...
    if actual != MATPAC_PLUS_CHECKPOINT_SHA256:
        raise ValueError(
            f"TinyMU checkpoint SHA-256 is {actual}, expected {MATPAC_PLUS_CHECKPOINT_SHA256}: "
            f"{resolved}"
        )
    return resolved


def resolve_matpac_plus_checkpoint(checkpoint: str = DEFAULT_MATPAC_PLUS_CHECKPOINT) -> Path:
    ...
    if checkpoint != DEFAULT_MATPAC_PLUS_CHECKPOINT:
        raise ValueError(
            f"MATPAC++ requires the pinned checkpoint URI {DEFAULT_MATPAC_PLUS_CHECKPOINT!r}, "
            f"got {checkpoint!r}"
        )

    cache_dir = embedding_model_dir(f"matpac-plus-{MATPAC_PLUS_CHECKPOINT_REVISION}")
    destination = cache_dir / MATPAC_PLUS_CHECKPOINT_NAME
    ...
    with tempfile.NamedTemporaryFile(
        dir=cache_dir, prefix=f".{MATPAC_PLUS_CHECKPOINT_NAME}.", delete=False
    ) as temporary:
...
def _validate_model_contract(model: _MatpacModel) -> None:
    ...
    expected = {
        "depth": MATPAC_PLUS_FRONTEND.encoder_depth,
        "embed_dim": MATPAC_PLUS_FRONTEND.base_embedding_dim,
        "n_freq": MATPAC_PLUS_FRONTEND.n_mels,
        "n_t": MATPAC_PLUS_FRONTEND.unit_frames,
        "patch_size": MATPAC_PLUS_FRONTEND.patch_size,
        "sample_rate": MATPAC_PLUS_FRONTEND.sample_rate,
    }
    if actual != expected:
        raise ValueError(f"MATPAC++ architecture is {actual}, expected {expected}")


def matpac_plus_num_latent_frames(num_samples: int, sample_rate: int) -> int:
    ...
    resampled_samples = math.ceil(num_samples * MATPAC_PLUS_FRONTEND.sample_rate / sample_rate)
    if resampled_samples < MATPAC_PLUS_FRONTEND.min_input_samples:
        raise ValueError(
            f"MATPAC++ needs at least {MATPAC_PLUS_FRONTEND.min_input_samples} samples after "
            f"resampling, got {resampled_samples}"
        )
    mel_frames = 1 + (
        resampled_samples - MATPAC_PLUS_FRONTEND.n_fft
    ) // MATPAC_PLUS_FRONTEND.hop_length
    return math.ceil(mel_frames / MATPAC_PLUS_FRONTEND.patch_size)


def matpac_plus_artifact_digest(checkpoint: str) -> str:
...
def encode_matpac_plus_column(
    ...
    :param encoder: Loaded MATPAC++ audio encoder.
    :returns: Fixed-shape Arrow tensor array with shape ``(3840, T_tokens)``.
    :raises ValueError: The encoder output violates MATPAC++'s shape or finite contract.
    """
    ...
    expected_shape = (
        ...
        matpac_plus_num_latent_frames(audio.shape[-1], sample_rate),
    )
    if embeddings.shape != expected_shape:
        raise ValueError(
            f"{MATPAC_PLUS_FIELD} encoder produced shape {embeddings.shape}, "
            f"expected {expected_shape}"
        )
    if not np.isfinite(embeddings).all():
        raise ValueError(f"{MATPAC_PLUS_FIELD} encoder produced non-finite values")
...
def matpac_plus_encoder_input(audio: np.ndarray, sample_rate: int) -> np.ndarray:
    ...
    """
    if audio.ndim != 3:
        raise ValueError(f"expected a (B, C, T) batch for MATPAC++, got shape {audio.shape}")
    if audio.shape[0] < 1:
        raise ValueError("MATPAC++ expects a non-empty batch")
    if audio.shape[1] not in (1, 2):
        raise ValueError(f"MATPAC++ expects 1 or 2 channels, got shape {audio.shape}")
    if sample_rate < 1:
        raise ValueError(f"MATPAC++ needs a positive sample_rate, got {sample_rate}")
    matpac_plus_num_latent_frames(audio.shape[-1], sample_rate)
    if not np.isfinite(audio).all():
        raise ValueError("MATPAC++ input audio contains non-finite values")
    peak_amplitude = float(np.max(np.abs(audio)))
    if peak_amplitude > 1.0:
        raise ValueError(
            f"MATPAC++ input audio is outside [-1.0, 1.0]: peak amplitude {peak_amplitude}"
        )

    mono = np.ascontiguousarray(audio.mean(axis=1, dtype=np.float32))
    if sample_rate == MATPAC_PLUS_FRONTEND.sample_rate:
        return mono
    ...
    resampled = audio_fn.resample(
        torch.from_numpy(mono), sample_rate, MATPAC_PLUS_FRONTEND.sample_rate
    )
    return np.ascontiguousarray(resampled.numpy(), dtype=np.float32)


def _load_matpac_plus_model(
    ...
    if missing != _EXPECTED_UNPERSISTED_BUFFERS or unexpected:
        raise ValueError(
            f"MATPAC++ checkpoint state is incompatible: missing={sorted(missing)}, "
            f"unexpected={sorted(unexpected)}"
        )
    return model.to(device).eval().requires_grad_(False)


def _encode_matpac_plus_chunk(
    ...
    expected = (
        len(chunk),
        matpac_plus_num_latent_frames(chunk.shape[-1], MATPAC_PLUS_FRONTEND.sample_rate),
        MATPAC_PLUS_FRONTEND.embedding_dim,
    )
    if tuple(embeddings.shape) != expected:
        raise ValueError(f"MATPAC++ encoder produced shape {tuple(embeddings.shape)}, expected {expected}")
    values = embeddings.float().cpu().numpy()
    if not np.isfinite(values).all():
        raise ValueError("MATPAC++ encoder produced non-finite values")
    return np.ascontiguousarray(values.transpose(0, 2, 1), dtype=np.float32)


def load_matpac_plus_audio_encoder(
    ...
    *,
    ...
    checkpoint_path = resolve_matpac_plus_checkpoint(checkpoint)
    model = _load_matpac_plus_model(
        cast("_MatpacFactory", matpac_wrapper), checkpoint_path, device
    )
    logger.info(
        "loaded_matpac_plus_checkpoint",
        checkpoint_revision=MATPAC_PLUS_CHECKPOINT_REVISION,
        checkpoint_sha256=MATPAC_PLUS_CHECKPOINT_SHA256,
        device=device,

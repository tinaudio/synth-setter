...
def render_param_spec_module(
    ...
    *,
    ...
    :param registered: ``True`` when the caller registers the spec itself
        (``--register``), so the header drops the manual-registration step.
    :returns: Python source for the draft spec module.
    """
    lines = _render_header(plugin_name, provenance, registered)
...
def _render_header(plugin_name: str, provenance: str | None, registered: bool) -> list[str]:
    ...
    :param registered: Switches the closing sentence between "register it" and "already
        registered".
    :returns: Source lines for the top of the draft module.
    """
    safe_name = _comment_safe(plugin_name).replace("\\", "/").replace('"', "'")
    registration_note = (
        "Registered in synth_setter.data.vst.param_spec_registry."
        if registered
        else "Then register the spec in synth_setter.data.vst.param_spec_registry."
    )
    lines = [

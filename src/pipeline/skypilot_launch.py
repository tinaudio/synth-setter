...
@click.option(
    ...
    default=None,
    help=(
        "Name for this launch. The launcher currently dispatches cluster-level "
        "(`sky.launch(cluster_name=...)`, `sky.tail_logs(cluster_name=...)`, `sky.down(...)`), "
        "so this value is the SkyPilot cluster name; it is also the R2 key prefix for the "
        "per-launch spec upload. Default: synth-setter-smoke-<config_id[:8]>. Must match "
...
def main(
    ...
    spec_uri = upload_spec_to_r2(spec, base_job_name)
